...
//private String pwd = "";
	...
	public ResultSet getParticipantsInAppointment(int appointmentId) throws SQLException{
		stmt = con.createStatement();
		String query = "select brukernavn, Godkjenning from Deltar_p� inner join Avtale on Deltar_p�.`AvtaleID` = Avtale.AvtaleID WHERE Deltar_p�.AvtaleID = "+appointmentId+";";
	...
	public ResultSet getInvitedAppointments(String username, Timestamp monday, Timestamp sunday) throws SQLException{
		stmt = con.createStatement();
		String query = "select * from Avtale inner join `Deltar_p�` on `Deltar_p�`.`AvtaleID` = `Avtale`.`AvtaleID` and brukernavn = '"+username+"' and (Starttid between '"+monday+"' and '"+sunday+"');";
		rs = stmt.executeQuery(query);
		return rs;
	}
	...
	public ResultSet getStatusForAppointment(String username, int appID) throws SQLException{
		stmt = con.createStatement();
		String query = "select brukernavn, Godkjenning from Deltar_p� WHERE brukernavn = '"+username+"' and AvtaleID = "+appID+";";
		rs = stmt.executeQuery(query);
		return rs;
	}
